...
declare class Class_DgramSocket extends Class_EventEmitter {
	 ...
	 * @async
	 */
	bind(port?: number/** = 0*/, addr?: string/** = ""*/, callback?:(err: Error, result: void)=>void): void;
	/**
	 * @brief 该方法会令 dgram.Socket 在 `opts` 指定的 `port` 和 `address` 上监听数据包信息。绑定完成时会触发一个 `listening` 事件。
	 * @param opts 指定绑定参数
	 * @async
	 */
	bind(opts: Fibjs.AnyObject, callback?:(err: Error, result: void)=>void): void;
	/**
	 ...
	 * @async
	 */
	send(msg: Class_Buffer, port: number, address?: string/** = ""*/, callback?:(err: Error, result: number)=>void): number;
	/**
	 * @brief 在 socket 上发送一个数据包
	 * @param msg 指定发送的数据
	 ...
	 * @async
	 */
	send(msg: Class_Buffer, offset: number, length: number, port: number, address?: string/** = ""*/, callback?:(err: Error, result: number)=>void): number;
	/**
