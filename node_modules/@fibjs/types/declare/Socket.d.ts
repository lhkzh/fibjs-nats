...
declare class Class_Socket extends Class_Stream {
	 ...
	 * @async
	 */
	connect(host: string, port: number, callback?:(err: Error, result: void)=>void): void;
	/**
	 ...
	 * @async
	 */
	recv(bytes?: number/** = -1*/, callback?:(err: Error, result: Class_Buffer)=>void): Class_Buffer;
	/**
	 * @brief 读取一个 UDP 包
	 ...
	 * @async
	 */
	recvfrom(bytes?: number/** = -1*/):{data:Class_Buffer,address:string,port:number};;
	/**
	 * @brief 将给定的数据写入连接，此方法等效于 write 方法
	 * @param data 给定要写入的数据
	 * @async
	 */
	send(data: Class_Buffer, callback?:(err: Error, result: void)=>void): void;
	/**
	 ...
	 * @async
	 */
	sendto(data: Class_Buffer, host: string, port: number, callback?:(err: Error, result: void)=>void): void;
} /** endof class */
